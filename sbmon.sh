...
update_cpu() {
	...
	CPU_PERCENT=$(echo "scale=2; $CPU_TICKS_BUSY_DIFF / $CPU_TICKS_TOTAL_DIFF * 100" | bc | sed 's/\..*//')
}
update_cpu
...
while true; do
	...
	if [[ "$SHOW_LABELS" == "1" ]]; then
		printf "%s\n" "CPU:${CPU_PERCENT_STR}$CPU_STR Mem:${MEM_PERCENT_STR}$MEM_STR Disk:${DISK_PERCENT_STR}$DISK_STR Net:${NET_PERCENT_STR}$NET_STR $CURRENT_TIME"
	else
		printf "%s\n" "${CPU_PERCENT_STR}$CPU_STR ${MEM_PERCENT_STR}$MEM_STR ${DISK_PERCENT_STR}$DISK_STR ${NET_PERCENT_STR}$NET_STR $CURRENT_TIME"
	fi
